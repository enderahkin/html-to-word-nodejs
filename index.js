(function () {
    ...
    const generateMarkup = (markupText, {
        headerTitle
    }) => {
        return `<html>
  <head>
  ...
  This is page 2 -->
    ...
    todoc = function (content, docname, _path) {
        return new Promise(async (resolve, reject) => {
            const inputMarkup = generateMarkup(data[0].Content, {
                docname
            })
            var outputFileName = `${docname || 'Report'}.docx`;
            var docx = HtmlDocx.asBlob(inputMarkup);
            fs.writeFile(_path + '.docx', docx, function (err) {
                if (err) {
                    reject(err)
                    return 0
                };
    ...
    module.exports = function (message, options) {
        return todoc(options.content,options.docname,options.path)
    };
})();